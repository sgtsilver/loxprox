...
- **AppSec http-scope plumbing moved to `/etc/nginx/conf.d/loxprox-appsec.conf`.** The `map $appsec_action $appsec_blocked { … }` and `log_format appsec_evt` directives that v1.4.0 added inline to the site file now live in a dedicated conf.d file, regenerated on every deploy when `ENABLE_APPSEC=true` (and removed when `false`). This lets the gateway add future AppSec audit-log features without touching the site config the operator edits.
