...
validate_ip() {
    [[ "$1" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]] || { error "Invalid IP: $1"; return 1; }
}

validate_network() {
    [[ "$1" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/[0-9]+$ ]] || { error "Invalid CIDR: $1"; return 1; }
}
...
configure_nginx() {
    ...
    if [[ "$ENABLE_APPSEC" == "true" ]]; then
        ...
        appsec_include='    include /etc/nginx/crowdsec-appsec.conf;'
        appsec_auth='
...
EOF
    ...
    if [[ "$ENABLE_APPSEC" == "true" ]]; then
        touch /etc/nginx/crowdsec-appsec.conf
    fi
...
configure_appsec_nginx() {
    ...
    cat > /etc/nginx/crowdsec-appsec.conf <<EOF
...
EOF
    ...
    chmod 640 /etc/nginx/crowdsec-appsec.conf
...
EOF
    ...
    info "Installing collections (hub-tracked)..."
    ...
    info "Upgrading all hub components to latest..."
...
rollback() {
    ...
    ok "Rollback from $latest complete."
}
