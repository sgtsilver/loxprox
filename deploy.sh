...
setup_gui() {
    case "${ENABLE_GUI,,}" in
        true|yes|1)
            ...
            local static_src="${SCRIPT_DIR:-.}/gui/static"
            if [[ ! -d "$static_src" ]]; then
                error "Panel assets not found at $static_src — cannot install the GUI."
                error "Run deploy.sh from a complete source tree (git clone), not a copied single file."
                return 1
            fi
        ...
        false|no|0|"")
            ...
            if [[ -f "$GUI_UNIT" ]]; then
                ...
                systemctl daemon-reload 2>/dev/null || true
                rm -f "$GUI_APP"
                rm -rf "$(dirname "$GUI_APP")/static"
                ok "LoxProx Panel removed (unit + $GUI_APP + assets deleted, firewall rule dropped)."
            fi
            ;;
        *)
            error "Invalid ENABLE_GUI value: '$ENABLE_GUI' (expected true/false)."
